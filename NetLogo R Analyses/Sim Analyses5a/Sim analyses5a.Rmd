...
#### Population
```{r}
null.2.0 <- glmer(cbind(resources.found, resources.untouched) ~ 1+(1|repetitions), 
                family = binomial(), data = all.data)
alt.2.0a <- glmer(cbind(resources.found, resources.untouched) ~ population+(1|repetitions), 
                family = binomial(), data = all.data)
anova(null.2.0, alt.2.0a, test="Chisq")
```
...
#### Communication
```{r}
null.3.0 <- glmer(cbind(resources.depleted, (resources.found-resources.depleted)) ~ 
                  1+(1|repetitions), 
                family = binomial(), data = all.data)
alt.3.0a <- glmer(cbind(resources.depleted, (resources.found-resources.depleted)) ~ 
                  communication+(1|repetitions), 
                family = binomial(), data = all.data)
anova(null.3.0, alt.3.0a, test="Chisq")
```
...
#### 1. Method
```{r}
null.4.0 <- lmer(energy.avg ~ 1+(1|repetitions), data = avg.data)
alt.4.0a <- lmer(energy.avg ~ id.method+(1|repetitions), data = avg.data)
anova(null.4.0, alt.4.0a)
```
...
#### Colony size 
```{r}
null.6.0 <- lmer(max.dist.km ~ 1+(1|repetitions), data = all.data)
alt.6.0a <- lmer(max.dist.km ~ population+(1|repetitions), data = all.data)
anova(null.6.0, alt.6.0a)
```

#### Communication
```{r}
alt.6.0b <- lmer(max.dist.km ~ communication+(1|repetitions), data = all.data)
anova(null.6.0, alt.6.0b)
```

#### Environment 
```{r}
alt.6.0c <- lmer(max.dist.km ~ environment+(1|repetitions), data = all.data)
anova(null.6.0, alt.6.0c)
```
