...
#### 1. Colony size and Method
```{r}
null <- lm(energy.avg ~ population+id.method+environment, data = avg.data)
alt.1a <- lm(energy.avg ~ population*id.method+environment, data = avg.data)
anova(null, alt.1a)
```

#### 2. Colony size and environment
```{r}
alt.1b <- lm(energy.avg ~ population*environment+id.method, data = avg.data)
anova(null, alt.1b)
```

#### 3. Method and environment
```{r}
alt.1c <- lm(energy.avg ~ population+id.method*environment, data = avg.data)
anova(null, alt.1c)
```

#### 4. Colony size, communication, and environment
```{r}
null.all.1 <- lm(energy.avg ~ population + id.method + environment + 
                   population*id.method + population*environment + id.method*environment, data = avg.data)
alt.all.1 <- lm(energy.avg ~ population*id.method*environment, data = avg.data)
anova(null.all.1, alt.all.1)
